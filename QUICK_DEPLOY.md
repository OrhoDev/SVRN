...
#### 2. Navigate to Relayer Directory
```bash
cd relayer
```
...
fly launch --name svrn-relayer --region iad --no-deploy
```

**When prompted:**
- **App name**: `svrn-relayer` (or choose your own)
- **Region**: `iad` (Washington, D.C. - closest to Solana devnet)
- **Postgres/Redis**: No (just press Enter)
...
This will:
- Build the app (detects Node.js automatically)
- Run `npm install` and `npm run build`
- Deploy to Fly.io
- Give you a URL like `https://svrn-relayer.fly.dev`

#### 6. Verify Relayer
```bash
curl https://svrn-relayer.fly.dev/health
...
**⚠️ IMPORTANT**: Replace `VITE_RELAYER_URL` with your actual Fly.io relayer URL (e.g., `https://svrn-relayer.fly.dev`)!
...
**"Failed to fetch"**
- Verify `VITE_RELAYER_URL` matches your Fly.io URL exactly (e.g., `https://svrn-relayer.fly.dev`)
- Check CORS settings (relayer should allow all origins)
...
2. **Monitor logs**: Check Fly.io logs (`fly logs`) and Vercel logs for any errors
