...
async function main() {
  ...
  console.log("🔧 Force Re-initializing Computation Definition...");
  ...
  if (compDefInfo) {
    console.log("📋 Current computation definition exists (", compDefInfo.data.length, "bytes)");
    console.log("   We'll reinitialize it...");
  }

  try {
    // Step 1: Reinitialize the computation definition
    // According to Arcium docs, calling init_comp_def again should work
    console.log("\n🔄 Step 1: Calling initAddTogetherCompDef to reinitialize...");
    const modifyComputeUnits = ComputeBudgetProgram.setComputeUnitLimit({
      units: 1_400_000,
    });
    
    // Try to call init even though it exists - Arcium should handle this
    try {
      ...
      if (initErr.message.includes("already in use") || initErr.message.includes("already initialized")) {
        console.log("⚠️  Computation definition already initialized (this is expected)");
        console.log("   Proceeding to upload circuit...");
      } else {
        throw initErr;
      }
    }

    // Step 2: Upload circuit - this is critical for on-chain storage
    console.log("\n🔄 Step 2: Uploading circuit (this links it to computation definition)...");
    ...
    try {
      await uploadCircuit(
        ...
        program.programId,
        rawCircuit,
        true, // finalize after upload - this is KEY
      );
      console.log("✅ Circuit uploaded and computation definition finalized!");
      console.log("   The computation definition is now properly linked and ready for use.");
    } catch (uploadErr: any) {
      if (uploadErr.message.includes("already in use")) {
        ...
        // Finalize to ensure proper linking even if circuit exists
    ...
    console.log("   The computation definition should now be properly initialized,");
    console.log("   linked to the circuit, and finalized.");
