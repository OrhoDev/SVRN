...
async function waitForComputation(connection: Connection, pda: PublicKey): Promise<Buffer> {
...
async function findNextComputationId(connection: Connection, clusterOffset: number): Promise<BN> {
    ...
    while (true) {
        ...
        id++;
        if (id > 1000) throw new Error("Too many computations pending cleanup");
    }
}
...
async function main() {
    ...
    // Load IDLs
    const solanaIdl = JSON.parse(fs.readFileSync('./idl.json', 'utf-8'));
    const arciumIdl = JSON.parse(fs.readFileSync('./arcium_idl.json', 'utf-8'));

    const solanaProgram = new Program(solanaIdl, provider) as any;
    const arciumProgram = new Program(arciumIdl, provider) as any;
    ...
    for (let i = 0; i < allVotes.length; i++) {
        ...
        try {
            ...
            const compPda = getComputationAccAddress(clusterOffset, compOffset);
